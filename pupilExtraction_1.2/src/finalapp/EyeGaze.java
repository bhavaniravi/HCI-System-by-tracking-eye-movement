...
public EyeGaze() {
  // Initialise all required matrices and cascades
    ...
    public Image processFrame(VideoCapture capture,boolean calibration,int method) {
       ...
       //Syntax -->  public  void detectMultiScale(Mat image, MatOfRect objects, double scaleFactor, int minNeighbors, int flags, Size minSize, Size maxSize)
        ...
        for (int i = 0; i < facesArray.length; i++){
           ...
           EyePoints p= find_pupil(mCascadeEL,eyearea_left,24,method,left);
            EyePoints p1=find_pupil(mCascadeER,eyearea_right,24,method,left);      
             if(framecount<15){
   ...
   private void CreateAuxiliaryMats() {
    ...
    private EyePoints  find_pupil(CascadeClassifier clasificator, Rect area,int size,int method,EyePoints p){
...
private void  gazePointer(ArrayList<EyePoints> points) {
        ...
        moveMouse(getMostOccoringElement(cell));
...
private int splitArea(Mat mROI,MinMaxLocResult p) {
        ...
        int cols=mROI.cols();
        Mat template=new Mat();
       area[0]=mROI.submat(new Range(3,rows/2),new Range(0,cols/2) );
       area[1]=mROI.submat(new Range(3,rows/2),new Range(cols/2,cols) );
       area[2]=mROI.submat(new Range(rows/2,rows),new Range(0,cols) );
